...
Generated package versions follow `<upstream-app-version>-uds.<uds-sub-version>`. The bridge infers the upstream version from the first service with a published port, or the first service when none are published. Tags that cannot be normalized to a semantic version, `latest`, digest-only images, and local builds fall back to `0.1.0-uds.0`. Set `x-uds.metadata.version: dev` for the standard UDS development workflow; the generated Zarf package version remains `dev`, while its Helm chart uses `0.0.0-dev` and retains the inferred upstream version as `appVersion`.
...
| `x-uds.metadata.version` | Package version override. `dev` is preserved for development packages, a semantic upstream version receives `-uds.0`, and an existing `<upstream>-uds.<sub-version>` value is preserved. |
