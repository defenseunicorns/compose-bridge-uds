...
Namespaced resources and internal namespace references use `{{ .Release.Namespace }}`; the chart does not create a Namespace, so Helm/Zarf controls the release namespace.
...
Package-owned secrets are rendered from chart values rather than baked into templates. Inline Compose config content is likewise exposed as an auto-indented deploy-time Zarf variable and rendered through chart values into a package-owned ConfigMap. Package-external secrets carry only non-sensitive Kubernetes Secret name and key variables; the chart neither includes their values nor creates their Secret objects. External Compose configs likewise carry non-sensitive Kubernetes ConfigMap name and key variables and do not create ConfigMaps. Service environment values are exposed as non-sensitive Zarf variables and rendered into per-service ConfigMaps. Every service also exposes a non-sensitive, auto-indented `<SERVICE>_ENV_FROM_SECRETS` YAML array for referencing existing Kubernetes Secrets (including ESO-managed Secrets) through `envFrom`; the generated package does not own those Secret objects or values. Each service's CPU and memory requests and limits are also exposed independently, with Compose reservations and limits supplying deployment defaults. Every package also exposes `APPLICATION_NAME` for the primary public identity, `DOMAIN` for generated endpoints and `ADDITIONAL_NETWORK_ALLOW` for deploy-time UDS network rules. The bridge writes `out/values/values.yaml` with `###ZARF_VAR_*###` placeholders for these deploy-time values, references it through `charts[].valuesFiles`, and retains their defaults, prompts, indentation, and sensitivity settings in the Zarf package's `variables:`.

## Inferred behavior

- **Expose:** Services with published `ports:` are exposed on the tenant gateway. The first inferred host uses deploy-time `APPLICATION_NAME`; additional hosts retain their service names. For multi-port services, the bridge prefers Compose `app_protocol` or `name` values indicating web traffic, then falls back to the first published port.
...
- **SSO:** A Keycloak client is generated for the first exposed service and omitted when no services are exposed. Its default name is `<APPLICATION_NAME> Login` and its client ID is `uds-<group>-<APPLICATION_NAME>`, with `compose` as the default group. Inferred Authservice redirect URIs use `https://<APPLICATION_NAME>.<DOMAIN>/auth/callback`; `DOMAIN` defaults to `uds.dev`.
...
- **Lifecycle hooks:** Supported `pre_start` hooks become per-Pod init containers, ordered after generated dependency waits.
